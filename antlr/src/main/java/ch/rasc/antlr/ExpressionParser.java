...
import ch.rasc.antlr.SimpleParser.ExprContext;

//http://blog.jdriven.com/2016/10/antlr-is-awesome/
public class ExpressionParser {
	...
	public int parse(final String expression) {
		...
		/*
		 * By default Antlr4 lexers / parsers have an attached error listener that prints
		 * errors to stderr. I prefer them to throw an exception instead so I implemented
		 * my own error listener which is attached here. I also remove any existing error
		 * listeners.
		 */
		lexer.removeErrorListeners();
		lexer.addErrorListener(this._listener);

		/*
		 * The lexer reads characters and lexes them into token stream. The tokens are
		 * consumed by the parser that then builds an Abstract Syntax Tree.
		 */
		final CommonTokenStream tokens = new CommonTokenStream(lexer);
		final SimpleParser parser = new SimpleParser(tokens);
		parser.removeErrorListeners();
		parser.addErrorListener(this._listener);
	...
	/*
	 * Helper method to create an ANTLRErrorListener. We're only interested in the
	 * syntaxError method.
	 */
	private static ANTLRErrorListener createErrorListener() {
		return new ANTLRErrorListener() {
			@Override
			public void syntaxError(final Recognizer<?, ?> arg0, final Object obj,
					final int line, final int position, final String message,
					final RecognitionException ex) {
				throw new IllegalArgumentException(String.format(Locale.ROOT,
						"Exception parsing expression: '%s' on line %s, position %s",
						message, line, position));
			}

			@Override
			public void reportContextSensitivity(final Parser arg0, final DFA arg1,
					final int arg2, final int arg3, final int arg4,
					final ATNConfigSet arg5) {
			}

			@Override
			public void reportAttemptingFullContext(final Parser arg0, final DFA arg1,
					final int arg2, final int arg3, final BitSet arg4,
					final ATNConfigSet arg5) {
			}

			@Override
			public void reportAmbiguity(final Parser arg0, final DFA arg1, final int arg2,
					final int arg3, final boolean arg4, final BitSet arg5,
					final ATNConfigSet arg6) {
			}
		};
	}
