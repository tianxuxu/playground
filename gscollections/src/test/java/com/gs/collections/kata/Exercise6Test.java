...
public class Exercise6Test extends CompanyDomainForKata {
	...
	public void sortedTotalOrderValue() {
		...
		Assert.assertEquals("Highest total order value", Double.valueOf(857.0),
				sortedTotalValues.getLast());
		Assert.assertEquals("Lowest total order value", Double.valueOf(71.0),
	...
	public void customerWithMaxTotalOrderValue() {
		...
		Assert.assertEquals(this.company.getCustomerNamed("Mary"),
	 ...
	 * <p>
	 * Hint: Use {@link RichIterable#forEach(Procedure)}. To solve the ambiguity error,
	 * use {@link Procedures#cast(Procedure)} .
	 *
	 * @see Order#deliver()
	 */
	@Test
	public void deliverOrdersToLondon() {
		Verify.assertAllSatisfy(this.company.getCustomerNamed("Fred").getOrders(),
				Order::isDelivered);
		Verify.assertNoneSatisfy(this.company.getCustomerNamed("Mary").getOrders(),
				Order::isDelivered);
		Verify.assertAllSatisfy(this.company.getCustomerNamed("Bill").getOrders(),
				Order::isDelivered);
	}
}
