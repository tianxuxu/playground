...
public class EncryptUploadObject {
	...
	NoSuchAlgorithmException, InvalidKeySpecException {
		if (args.length == 5) {
			...
			SecretKey mySymmetricKey = createSecretKey(password, bucketName, file.getFileName().toString());

			EncryptionMaterials materials = new EncryptionMaterials(mySymmetricKey);

			AWSCredentials credentials = new BasicAWSCredentials(args[0], args[1]);

			AmazonS3EncryptionClient client = new AmazonS3EncryptionClient(credentials, materials);

			TransferManager tm = new TransferManager(client);
			Upload upload = tm.upload(bucketName, file.getFileName().toString(), file.toFile());
			try {
			...
			System.out.println("java -jar s3backup.jar " + EncryptUploadObject.class.getName()
					+ " accessKey secretKey password bucketName pathToFile");
		}

	}

	public static SecretKey createSecretKey(String password, String bucketName, String objectName)
