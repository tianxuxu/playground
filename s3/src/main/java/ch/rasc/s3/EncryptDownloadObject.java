...
public class EncryptDownloadObject {
	...
	NoSuchAlgorithmException, InvalidKeySpecException {
		if (args.length == 6) {
			...
			SecretKey mySymmetricKey = EncryptUploadObject.createSecretKey(password, bucketName, objectKey);
			...
			try {
				...
				System.out.println("Unable to download file, download was aborted: " + objectKey);
				e.printStackTrace();
			}

		} else {
			System.out.println("java -jar s3backup.jar " + EncryptDownloadObject.class.getName()
