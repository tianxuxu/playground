Ext.define('Ext.ux.Atmosphere', {
    ...
    subscribe: function(url, callback, request) {
        ...
        Ext.ux.Atmosphere.requests[Ext.ux.Atmosphere.requests.length] = rq;
        return rq;
    },

    addCallback: function(func) {
        if (Ext.Array.contains(func, Ext.ux.Atmosphere.callbacks)) {
            Ext.ux.Atmosphere.callbacks.push(func);
        }
    },

    removeCallback: function(func) {
        var index = Ext.Array.contains(func, Ext.ux.Atmosphere.callbacks);
        if (index != -1) {
            Ext.ux.Atmosphere.callbacks.splice(index, 1);
        }
    },

    unsubscribe : function() {
        if (Ext.ux.Atmosphere.requests.length > 0) {
            for (var i = 0; i < Ext.ux.Atmosphere.requests.length; i++) {
                Ext.ux.Atmosphere.requests[i].close();
                clearTimeout(Ext.ux.Atmosphere.requests[i].id);
            }
        }
        Ext.ux.Atmosphere.requests = [];
        Ext.ux.Atmosphere.callbacks = [];
    },
    ...
    unsubscribeUrl: function(url) {
        ...
        if (Ext.ux.Atmosphere.requests.length > 0) {
            for (var i = 0; i < Ext.ux.Atmosphere.requests.length; i++) {
                var rq = Ext.ux.Atmosphere.requests[i];
        ...
        if (idx >= 0) {
            Ext.ux.Atmosphere.requests.splice(idx, 1);
        }
    },
    ...
    AtmosphereRequest : function(options) {
        ...
        function _execute() {
            if (_request.transport != 'websocket') {
                ...
                if (!_supportWebsocket()) {
                	Ext.log({level: _request.logLevel}, "Websocket is not supported, using request.fallbackTransport (" + _request.fallbackTransport + ")");
        ...
        function _buildWebSocketUrl() {
            ...
            if (url.indexOf("http") == -1 && url.indexOf("ws") == -1) {
                url = Ext.ux.Atmosphere.parseUri(document.location, url);
            }
        ...
        function _executeWebSocket(webSocketOpened) {
            ...
            Ext.log({level: _request.logLevel}, "Invoking executeWebSocket");
            if (_request.logLevel == 'debug') {
                Ext.log.debug("Using URL: " + location);
            }
            ...
            _websocket.onopen = function(message) {
                if (_request.logLevel == 'debug') {
                    Ext.log.debug("Websocket successfully opened");
                }
            ...
            _websocket.onerror = function(message) {
                Ext.log.warn("Websocket error, reason: " + message.reason);
            ...
            _websocket.onclose = function(message) {
                ...
                Ext.log.warn("Websocket closed, reason: " + reason);
                Ext.log.warn("Websocket closed, wasClean: " + message.wasClean);
                ...
                if (_abordingConnection) {
                	...
                	Ext.log({level: _request.logLevel},"Websocket failed. Downgrading to Comet and resending");
                    _open('opening', _request.fallbackTransport);
                    _reconnectWithFallbackTransport();
        ...
        function _attachHeaders(request) {
            ...
            url += "&X-Atmosphere-Framework=" + Ext.ux.Atmosphere.version();
            ...
            Ext.iterate(rq.headers, function(name, value) {
                var h = Ext.isFunction(value) ? value.call(this, ajaxRequest, request, create) : value;
                if (h) {
        ...
        function _executeRequest(request) {
            ...
            if ((rq.transport == 'jsonp') || ((rq.enableXDR) && (this.checkCORSSupport()))) {
                _jsonp(rq);
                return;
            }
        ...
        function _doRequest(ajaxRequest, request, create) {
            ...
            var url = Ext.ux.Atmosphere.prepareURL(request.url);
            ...
            ajaxRequest.setRequestHeader("X-Atmosphere-Framework", Ext.ux.Atmosphere.version());
        ...
        function _configureIE(request) {
            ...
            return {
                open: function() {
                    ...
                    url = Ext.ux.Atmosphere.prepareURL(url);
                    ...
                    stop = Ext.ux.Atmosphere.iterate(function() {
                        if (!cdoc.firstChild) {
                            return;
                        }
        ...
        function _pushWebSocket(message) {
            ...
            try {
            	...
            	Ext.log({level: _request.logLevel}, "Websocket failed. Downgrading to Comet and resending " + data);
        ...
        function _invokeCallback() {
            ...
            if (Ext.ux.Atmosphere.callbacks.length > 0) {
                Ext.log.debug("Invoking " + Ext.ux.Atmosphere.callbacks.length + " global callbacks: " + _response.state);
                try {
                    Ext.iterate(Ext.ux.Atmosphere.callbacks, call);
                } catch (e) {
            ...
            if (typeof(_request.callback) == 'function') {
                if (_request.logLevel == 'debug') {
                	Ext.log.debug("Invoking request callbacks");
                }
                try {
