...
public class JsonFile {
	...
	private static String toJson(ImmutableGocument document) {
		return new GsonBuilder().registerTypeAdapterFactory(new GsonAdaptersGocument())
	...
	private static org.immutables.samples.json.immutables.Gocument.Item createItem(int iteration, boolean recurse) {
		...
		builder.id(iteration + 1000).name(textFor(iteration % 15, 10))
				...
				.bar(iteration % 2 == 0).tid(iteration + 528)
				.gname(textFor(iteration % 15, 10)).bdescription(textFor(iteration, 40))
				.hfoo(iteration * 13).ybar(iteration % 3 == 0);
