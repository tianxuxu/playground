...
public class PDFTableGenerator {
	...
	public void drawTable(PDDocument doc, Table table) throws IOException {
		...
		Integer rowsPerPage = new Double(
				Math.floor(table.getHeight() / table.getRowHeight())).intValue() - 1; // subtract
		...
		for (int pageCount = 0; pageCount < numberOfPages; pageCount++) {
			...
			try (PDPageContentStream contentStream = generateContentStream(doc, page,
					table)) {
	...
	private static void drawCurrentPage(Table table, String[][] currentPageContent,
		...
		float nextTextY = tableTopY - (table.getRowHeight() / 2)
				- ((table.getTextFont().getFontDescriptor().getFontBoundingBox()
						.getHeight() / 1000 * table.getFontSize()) / 4);

		// Write column headers
		writeContentLine(table.getColumnsNamesAsArray(), contentStream, nextTextX,
				nextTextY, table);
	...
	private static void writeContentLine(String[] lineContent,
			PDPageContentStream contentStream, float nextTextX, float nextTextY,
	...
	private static PDPageContentStream generateContentStream(PDDocument doc, PDPage page,
			Table table) throws IOException {
		PDPageContentStream contentStream = new PDPageContentStream(doc, page,
				AppendMode.OVERWRITE, false, false);
		...
		if (table.isLandscape()) {
			contentStream.transform(
					new Matrix(0, 1, -1, 0, table.getPageSize().getWidth(), 0));
		}
