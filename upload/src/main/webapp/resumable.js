...
var Resumable = function(opts) {
	...
	// - slicing files
	this.support = ((typeof (File) !== 'undefined') && (typeof (Blob) !== 'undefined')
			&& (typeof (FileList) !== 'undefined') && (!!Blob.prototype.webkitSlice || !!Blob.prototype.mozSlice
	...
	// EVENTS
	// catchAll(event, ...)
	// fileSuccess(file), fileProgress(file), fileAdded(file), fileRetry(file),
	...
	$h = {
		...
		generateUniqueIdentifier: function(file) {
			var fileName = file.fileName || file.name; // Some confusion in
	...
	function ResumableFile(resumableObj, file) {
		...
		$.progress = function() {
			...
			ret = Math.max($._prevProgress, ret); // We don't want to loose
													// percentages when an
													// upload is paused
			$._prevProgress = ret;
			return (ret);
	...
	function ResumableChunk(resumableObj, fileObj, offset, callback) {
		...
		// test() makes a GET request without any data to see if the chunk has
		// already been uploaded in a previous session
		$.test = function() {
			// Set up request and listen for event
			$.xhr = new XMLHttpRequest();
			$.xhr.timeout = 5000; // set a fairly low threshold before server
									// must respond, since we'll just be
									// retrying
		...
		$.send = function() {
			...
			var doneHandler = function(e) {
				...
				if (status == 'success' || status == 'error') {
					...
					$.abort();
					$.send(); // TODO: Insert a retryInterval pause into this
								// loop
				}
			};
		...
		$.status = function() {
			...
			if (!$.xhr) {
				...
				// Status is really 'OPENED', 'HEADERS_RECEIVED' or 'LOADING' -
				// meaning that stuff is happening
				return ('uploading');
			} else {
				if ($.xhr.status == 200) {
					// HTTP 200, perfect
	...
	$h.uploadNextChunk = function() {
		...
		// The are no more outstanding chunks to upload, check is everything is
		// done
		$h.each($.files, function(file) {
			outstanding = false;
			$h.each(file.chunks, function(chunk) {
	...
	$.assignBrowse = function(domNodes) {
		...
		// we haven't a choice.
		// FF4+ allows click() for this though:
		// https://developer.mozilla.org/en/using_files_from_web_applications)
		$h.each(domNodes, function(domNode) {
			var input = document.createElement('input');
			input.setAttribute('type', 'file');
			input.setAttribute('multiple', 'multiple');
			// Place <input multiple /> with the dom node an position the input
